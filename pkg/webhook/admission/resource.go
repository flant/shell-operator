...
func (w *ValidatingWebhookResource) AddWebhook(config *ValidatingWebhookConfig) {
	w.Webhooks[config.Metadata.WebhookId] = config
}

func (w *ValidatingWebhookResource) CreateConfiguration() error {
	...
	for _, webhook := range w.Webhooks {
		...
		webhook.ClientConfig = v1.WebhookClientConfig{
			Service: &v1.ServiceReference{
				...
				Path:      createWebhookPath(webhook),
			},
...
func (w *ValidatingWebhookResource) UpdateConfiguration() error {
	return nil
}

func (w *ValidatingWebhookResource) DeleteConfiguration() error {
...
func (w *ValidatingWebhookResource) CreateOrUpdateConfiguration(conf *v1.ValidatingWebhookConfiguration) error {
