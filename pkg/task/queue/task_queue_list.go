...
func (q *TaskQueue) compaction() {
	...
	for _, group := range hookGroups {
		...
		targetContexts := targetMetadata.(task_metadata.BindingContextAccessor).GetBindingContext()
		targetMonitorIDs := targetMetadata.(task_metadata.MonitorIDAccessor).GetMonitorIDs()
