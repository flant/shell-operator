...
// Test Filter function - main entry point
func TestRun(t *testing.T) {
	t.Run("with custom filter function - success", func(t *testing.T) {
		...
		result, err := Run(nil, filterFn, obj)
	...
	t.Run("with custom filter function - error", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Nil(t, result)
		assert.Contains(t, err.Error(), "filterFn")
	...
	t.Run("with jq expression - simple filter", func(t *testing.T) {
		...
		expr, err := CompileExpression(".spec.nodeName")
		require.NoError(t, err)

		result, err := Run(expr, nil, obj)
		require.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, `"node-1"`, result.FilterResult)
		assert.NotEmpty(t, result.Metadata.Checksum)
	...
	t.Run("with jq expression - complex filter", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, `["nginx","sidecar"]`, result.FilterResult)
	})
	...
	t.Run("with jq expression - filter returns null", func(t *testing.T) {
		...
		expr, err := CompileExpression(".spec.nonexistent")
		require.NoError(t, err)

		result, err := Run(expr, nil, obj)
		require.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, "null", result.FilterResult)
	})
	...
	t.Run("with jq expression - error", func(t *testing.T) {
		...
		expr, err := CompileExpression(".spec.field")
		require.NoError(t, err)
...
func BenchmarkRun(b *testing.B) {
	...
	b.Run("CustomFilter", func(b *testing.B) {
		filterFn := func(o *unstructured.Unstructured) (interface{}, error) {
			return map[string]interface{}{"name": o.GetName()}, nil
		}
		for i := 0; i < b.N; i++ {
			_, err := Run(nil, filterFn, obj)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("JQFilter", func(b *testing.B) {
	...
	b.Run("JQFilterComplex", func(b *testing.B) {
		expr, err := CompileExpression(".spec.containers[].name")
		if err != nil {
			b.Fatal(err)
		}
